...
- (void)onTapMediaItemSendGift:(NIMMediaItem *)item
{
    
    NIMGiftListView *view = [NIMGiftListView instanceTextView];
    view.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height);
    view.myPrice = self.myPrice;
    view.array = self.giftArray;
    [self.view addSubview:view];
    view.sendGiftActionBlock = ^(NSMutableDictionary *dict){
        NSLog(@"dict====%@",dict);
        
        [self deletePrice:dict block:^(NSString *str) {
            if ([str isEqualToString:@"0"]) {
                [self.interactor mediaGiftPressed:dict];
            }else {
                NSLog(@"金币不足");
...
//删除金币
-(void)deletePrice:(NSMutableDictionary *)dict block:(void (^)(NSString *))block {
...
- (void)setUpTitleView
{
    ...
    self.subTitleLabel.font = [UIFont systemFontOfSize:20.f];
