...
- (void)viewDidLoad{
    ...
    self.emptyTipView = [[UIView alloc]initWithFrame:self.view.bounds];
    self.emptyTipView .backgroundColor = [UIColor whiteColor];
    [self.emptyTipView  addSubview:self.emptyTipLabel];
    
    self.emptyTipLabel = [[UILabel alloc] init];
    self.emptyTipLabel.text = @"还没有会话，在首页找个人聊聊吧";
...
- (void)refreshSubview{
    ...
    self.emptyTipLabel.centerY = self.tableView.height * .5f - 100;
}
...
- (UIView*)titleView:(NSString*)userID{
    ...
    subLabel.textColor = [UIColor whiteColor];//RGBA(255, g: 255, b: 255, a: 1)
...
- (NSAttributedString *)contentForRecentSession:(NIMRecentSession *)recent{
    ...
    if (recent.lastMessage.messageType == NIMMessageTypeCustom)
    {
        ...
        if ([object.attachment isKindOfClass:[NTESSnapchatAttachment class]]) {
            ...
            text = @"[白板]";
        }else if ([object.attachment isKindOfClass:[NTESSendGiftAttachment class]]) {
            text = @"[礼物]";
        }else{
            text = @"[未知消息]";
        }
