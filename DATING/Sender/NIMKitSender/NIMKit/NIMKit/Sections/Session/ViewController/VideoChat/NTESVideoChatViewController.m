...
@property (nonatomic,strong) NSString   *oldDurationDesc;
...
- (void)viewDidLoad {
    ...
    self.oldDurationDesc = @"-1";
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hangupVideo) name:@"hangupVideo" object:nil];
    [super viewDidLoad];
    if (self.localPreView) {
...
- (void)onNTESTimerFired:(NTESTimerHolder *)holder{
    ...
    self.durationLabel.text = self.durationDesc;
    NSLog(@"self.durationDesc===%@",self.durationDesc); //01:52
    
    NSArray *array = [self.durationDesc componentsSeparatedByString:@":"];
    if ([array[0] isEqualToString:@"00"]  && [array[1] isEqualToString:@"00"]) {
        self.contentMoney(1);
    }
    if ([self.oldDurationDesc intValue] + nim  < [array[0] intValue] ) {
        // 扣金币
        self.oldDurationDesc = array[0];
        NSLog(@"到3分钟了，视频扣金币");
        NSLog(@"视频扣金币");
        NSLog(@"self.oldDurationDesc====\%@",self.oldDurationDesc);
        self.contentMoney(0);
    }
}
