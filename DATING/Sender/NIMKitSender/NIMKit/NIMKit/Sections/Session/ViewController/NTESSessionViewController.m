...
-(void)openMediaItemAudioChat:(NIMMediaItem *)item{
    [self onTapMediaItemAudioChat:item];
}
-(void)openMediaItemVideoChat:(NIMMediaItem *)item{
    [self onTapMediaItemVideoChat:item];
}
...
- (void)onTapMediaItemAudioChat:(NIMMediaItem *)item
{
    if ([self checkRTSCondition]) {
        ...
        vc.contentMoney = ^(int time){
            if (time == 1) {
                // 扣除一分钟
                [self deleteMoney:(Video) time:1 block:^{
                    NSLog(@"没钱了，挂断语音");
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"hangupAudio" object:nil];
                }];
            }else {
                [self deleteMoney:(Video) time:0 block:^{
                    NSLog(@"没钱了，挂断语音");
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"hangupAudio" object:nil];
                }];
            }
    ...
    if ([self checkRTSCondition]) {
        ...
        vc.contentMoney = ^(int time){
            if (time == 1) {
                ...
                [self deleteMoney:(Video) time:1 block:^{
                    NSLog(@"没钱了，挂断语音");
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"hangupVideo" object:nil];
                }];
            }else {
                // 扣费
                [self deleteMoney:(Video) time:0 block:^{
...
- (BOOL)onTapCell:(NIMKitEvent *)event
{
    ...
    else if([eventName isEqualToString:@"NIMKitEventNameTapGift"])
    {
...
- (void)showCustom:(NIMMessage *)message
{
    ...
    if ([object.attachment isKindOfClass:[NTESSendGiftAttachment class]] ){
        NTESSendGiftAttachment *a = (NTESSendGiftAttachment *)object.attachment;
        NSDictionary *dict = a.messageDic;
        if (dict == nil) {
//            NSDictionary *dict2 = (NSDictionary *)object;
//            if (dict2 != nil ) {
//                DATGiftDetailView *view = [DATGiftDetailView instanceTextView];
//                view.dictStr = dict2;
...
//            }
            ...
            dict[@"icon"] = a.headerImage;
            dict[@"name"] = a.nameLabel;
            dict[@"price"] = a.priceLabel;
            DATGiftDetailView *view = [DATGiftDetailView instanceTextView];
...
- (void)showGift:(NIMMessage *)message
{
    //普通的自定义消息点击事件可以在这里做哦~
    NSLog(@"礼物的点击事件");
...
//    }
    ...
    [moreBtn addTarget:self action:@selector(enterMoreCard:) forControlEvents:UIControlEventTouchUpInside];
    [moreBtn setImage:[UIImage imageNamed:@"gaitubao_com"] forState:UIControlStateNormal];
    [moreBtn setImage:[UIImage imageNamed:@"gaitubao_com"] forState:UIControlStateHighlighted];
    [moreBtn sizeToFit];
    UIBarButtonItem *robotInfoButtonItem = [[UIBarButtonItem alloc] initWithCustomView:moreBtn];
